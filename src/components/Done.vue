<template>
  ...
  <div class="card-header" style="background-color:#0c6;color:white">{{name}}</div>
  <div class="card-body">
    <div class="card border-light mb-3" style="max-width: 20rem;" v-for="(index,i) in done" :key="index.title">
        ...
        <div class="modal" :id="'modal'+i">
            <div class="modal-dialog" role="document">
                ...
                <div class="modal-footer" v-if="token">
                    <button type="button" class="btn btn-primary" v-on:click="back(i,index)" >Back to Doing</button>
...
export default {
  ...
  methods: {
    destroy (index) {
      kanban.child('done').child(index).set(null)
