<template>
  ...
  <div class="card-header" style="background-color:#008cba;color:white">{{name}}</div>
  <div class="card-body">
    <div class="card border-light mb-3" style="max-width: 20rem;" v-for="(index,i) in doing" :key="index.title">
        ...
        <div class="modal" :id="'modal'+i">
            <div class="modal-dialog" role="document">
                ...
                <div class="modal-footer" v-if="token">
                    <button type="button" class="btn btn-primary" v-on:click="back(i,index)" >Back to Todo</button>
                    <button type="button" class="btn btn-secondary" v-on:click="destroy(i)" >Delete</button>
                    <button type="button" class="btn btn-primary" v-on:click="next(i,index)">To Done</button>
                </div>
                </div>
            </div>
        </div>
...
export default {
  ...
  methods: {
    destroy (index) {
      kanban.child('doing').child(index).set(null)
      document.getElementById(`modal${index}`).modal('hide')
    },
    next (index, data) {
      kanban.child('doing').child(index).set(null)
      kanban.child('done').push(data)
      document.getElementById(`modal${index}`).modal('hide')
    },
    back (index, data) {
      kanban.child('doing').child(index).set(null)
